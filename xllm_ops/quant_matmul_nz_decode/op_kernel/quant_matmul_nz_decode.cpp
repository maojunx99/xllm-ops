...
template <uint32_t kL1TileN, uint32_t kL1TileK,
          uint32_t kL0TileN = 128, uint32_t kL0TileK = 256,
          bool kEnableShuffleK = false>
__aicore__ inline void RunQuantMatmulNzDecode(
    GM_ADDR x, GM_ADDR weight, GM_ADDR scale, GM_ADDR bias, GM_ADDR y,
  ...
  using BroadcastOneBlkType =
      Gemm::GemmType<float, layout::RowMajor>;
  using ColumnBroadcastMulType =
      Gemm::GemmType<float, layout::RowMajor>;
  using EpilogueTileShape = MatrixShape<16, 256>;
  using TileRowBroadcastMul = Epilogue::Tile::TileRowBroadcastMul<
      ArchTag, RowBroadcastMulType, EpilogueTileShape>;
  using TileBroadcastOneBlk = Epilogue::Tile::TileBroadcastOneBlk<
      ArchTag, BroadcastOneBlkType, EpilogueTileShape::ROW>;
  using TileColumnBroadcastMul =
      Epilogue::Tile::TileOneBlkColumnBroadcastMul<
          ArchTag, ColumnBroadcastMulType, EpilogueTileShape>;
  using TileCopy = Epilogue::Tile::TileCopy<
      ArchTag, CType, ScaleType, PerTokenScaleType, DType>;
  using EpilogueScheduler =
      Epilogue::Tile::EpilogueHorizontalTileSwizzle;
  using BlockEpilogue = Epilogue::Block::BlockEpilogue<
      EpiloguePolicy, CType, ScaleType, PerTokenScaleType, DType,
      TileRowBroadcastMul, TileBroadcastOneBlk, TileColumnBroadcastMul,
      TileCopy, EpilogueScheduler>;
  using BlockScheduler = Gemm::Block::GemmIdentityBlockSwizzle<3, 1>;
  using MatmulKernel = Gemm::Kernel::QuantMatmulNzWorkspace<
      BlockMmad, BlockEpilogue, BlockScheduler, 2>;
...
extern "C" __global__ __aicore__ void quant_matmul_nz_decode(
    GM_ADDR x, GM_ADDR weight, GM_ADDR scale, GM_ADDR bias, GM_ADDR y,
...
__aicore__ inline void PrepareQuantMatmulNzMixedCoreWorkspace(
    __gm__ uint8_t*) {
...
#define clearWorkspace(workspace) PrepareQuantMatmulNzMixedCoreWorkspace(workspace)
