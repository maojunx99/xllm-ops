...
private:
        ...
        AscendC::GlobalTensor<ElementC> const &gm_block_c)
    {
        constexpr bool kSupportsTwoDimensionalEpilogue =
            L1TileShape::N == 128 || L1TileShape::N == 256;
        constexpr uint32_t kMinTwoDimensionalRows =
            L1TileShape::N == 128 ? 2 : 8;
        ...
        static_epilogue_row(params, block_coord, actual_block_shape,
                            gm_block_c);
    }

    CATLASS_DEVICE
    void static_epilogue_row(
        Params const &params,
        ...
        AscendC::GlobalTensor<ElementC> const &gm_block_c)
    {
        ...
        auto ub_bias =
            resource.ubBuf.template GetBufferByByte<int32_t>(ub_offset);
        ub_offset += kEpilogueTileN * sizeof(int32_t);
        auto ub_scale =
            resource.ubBuf.template GetBufferByByte<float>(ub_offset);
        ub_offset += kEpilogueTileN * sizeof(float);
        auto ub_float =
            resource.ubBuf.template GetBufferByByte<float>(ub_offset);
        ub_offset += kMaxElements * sizeof(float);
        auto ub_d =
            resource.ubBuf.template GetBufferByByte<ElementD>(ub_offset);
        ...
        const uint16_t accumulator_burst_length = static_cast<uint16_t>(
            tile_n * sizeof(int32_t) / kDataBlockBytes);
        ...
        for (uint32_t column_offset = 0; column_offset < tile_n;
            ...
            AscendC::Add(ub_c[column_offset], ub_c[column_offset],
                         ub_bias[column_offset], vector_elements,
                         static_cast<uint8_t>(row_count), broadcast_params);
        }
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Cast(ub_float, ub_c, AscendC::RoundMode::CAST_RINT,
                      element_count);
        AscendC::PipeBarrier<PIPE_V>();
        for (uint32_t column_offset = 0; column_offset < tile_n;
             column_offset += kVectorRepeatElements) {
            const uint32_t vector_elements =
                tile_n - column_offset < kVectorRepeatElements
                    ? tile_n - column_offset
                    : kVectorRepeatElements;
            AscendC::Mul(ub_float[column_offset], ub_float[column_offset],
                         ub_scale[column_offset], vector_elements,
                         static_cast<uint8_t>(row_count), broadcast_params);
        }
        AscendC::PipeBarrier<PIPE_V>();
        AscendC::Cast(ub_d, ub_float, AscendC::RoundMode::CAST_RINT,
                      element_count);

        AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(EVENT_ID0);
        AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(EVENT_ID0);
        const uint16_t output_burst_length = static_cast<uint16_t>(
            tile_n * sizeof(ElementD) / kDataBlockBytes);
        const uint16_t output_destination_gap = static_cast<uint16_t>(
            (params.problemShape.n() - tile_n) * sizeof(ElementD) /
            kDataBlockBytes);
        AscendC::DataCopy(
            gm_d[block_n_offset], ub_d,
            AscendC::DataCopyParams{static_cast<uint16_t>(row_count),
    ...
    struct AicWaitFunc {
        using MatmulKernel = QuantMatmulNzWorkspace<BlockMmad, BlockEpilogue, BlockScheduler,
            WORKSPACE_STAGES>;
